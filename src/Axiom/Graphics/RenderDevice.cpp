...
namespace Ax
{
	...
	bool RenderContext::CreateRenderContext(AdapterData* adapter)
	{
		...
		HRESULT Hr = D3D11CreateDevice(adapter != nullptr ? adapter->Adapter : nullptr, DriverType, nullptr, DeviceCreationFlags, FeatureLevels, sizeof(FeatureLevels) / sizeof(D3D_FEATURE_LEVEL), D3D11_SDK_VERSION, &Device, &CreatedFeatureLevel, &DeviceContext);
		return !FAILED(Hr);
	}
	...
	ID3DBlob* RenderContext::CompileShader(const ShaderCompileDesc& desc) const
	{
		...
		HRESULT hr = D3DCompile(desc.Source.CStr(), desc.Source.Length(), desc.Name.CStr(), macros, D3D_COMPILE_STANDARD_FILE_INCLUDE, desc.EntryPoint.CStr(), desc.Profile.CStr(), flags, 0, &shaderBlob, &errorBlob);
