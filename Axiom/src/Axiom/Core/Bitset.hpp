...
struct Bitset128
{
	...
	Bitset128(uint64 a, uint64 b) { bits[0] = a; bits[1] = b; }
...
AX_ALIGNAS(32) struct Bitset256
{
	...
	Bitset256(uint64 r) { bits[0] = r; bits[1] = r; bits[2] = r; bits[3] = r; }
	Bitset256(uint64 a, uint64 b, uint64 c, uint64 d) { bits[0] = a; bits[1] = b; bits[2] = c; bits[3] = d; }
...
AX_ALIGNAS(32) struct Bitset512
{
	union {
	...
	Bitset512(unsigned long repeat) { Algorithms::FillN(bits, 8, repeat); }
...
AX_ALIGNAS(32) struct Bitset1024
{
	union {
	...
	Bitset1024(unsigned long repeat) { Algorithms::FillN(bits, 16, repeat); }
...
AX_ALIGNAS(32) struct Bitset2048
{
	union {
	...
	Bitset2048(unsigned long repeat) { Algorithms::FillN(bits, 32, repeat); }
...
AX_ALIGNAS(32) struct Bitset4096
{
	...
	Bitset4096(unsigned long repeat) { Algorithms::FillN(bits, 32, repeat); }
