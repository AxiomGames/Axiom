...
namespace ax
{
	...
	{
		void Execute() // overrided function
		{
		...
		{
			m_Data = data; m_Len = len; m_Func = func;
		}
	...
	public:
		...
		static void ExecuteOnce(const int numThreads, T* data, const int size, std::function<void(T&)> f, bool waitUntilFinish = true)
		{
			...
			if (waitUntilFinish)
			{
				while (data < end) { // work in this thread too
					f(*data++);
				}
		...
		const uint32 GetNumThreads() const { return m_NumThreads; }
